...
export const InputButtons = ({ widget_area_title, onChange, onClick }) => {

  let button_text = 'Save';
  if ( widget_area_title !== '' ) {
    button_text = 'Update';
  }
  return (
    <div className="uo-tclr-gutenberg-content-actions">
      <input
        value={ widget_area_title }
        onChange={ onChange }
        type="text"
      />
      <Button
...
// Customizer view button
export const CustomizerButton = ({ widget_area_title }) => {
  ...
  let customizer_link = window.location.origin
    + '/wp-admin/customize.php?url='
    + encodeURIComponent( wp.data.select('core/editor').getPermalink() );
  let button_text = 'Customizer View';

  if (widget_area_title !== '') {
    customizer_link = customizer_link+'&autofocus[section]=sidebar-widgets-' + wpFeSanitizeTitle( widget_area_title );
    return (
      <div className="uo-tclr-gutenberg-content-actions">
        <Button
          isPrimary
          href = {customizer_link}
        >
          {__(button_text)}
        </Button>
      </div>
